...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final ArrayList<String> subjectList = foodManager.getFoodList();
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                String subject = foodManager.getItem(position);
        ...
        listView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                foodManager.removeData(position); //이것만 써주면 원본데이터에서만 사라지고 화면에서는 사라지지 않아서
    ...
    public void onClick(View v){
        switch (v.getId()){
            case R.id.btnInsert:
                ...
                foodManager.addData(data); //이것만 써주면 원본데이터가 화면에 반영이 안됨.
                adapter.notifyDataSetChanged(); //반드시 원본데이터가 바뀌었음을 화면에 알려줌
                break;

            case R.id.btnUpdate:
                String updateData = editText.getText().toString();
                foodManager.updateData(selectedPos, updateData);//원본 데이터 수정 (위치, 바꿀내용)
