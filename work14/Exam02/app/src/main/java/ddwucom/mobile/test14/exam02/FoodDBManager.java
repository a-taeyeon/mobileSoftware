...
//    _id 를 기준으로 food 의 이름과 nation 변경
    public boolean modifyFood(Food food) {
        ...
        row.put(FoodDBHelper.COL_FOOD, food.getFood());   //foodName이 food.getFood()로 바뀜
        row.put(FoodDBHelper.COL_NATION, food.getNation());   //nation이 food.getNation()으로 바뀜

        String whereClaus = FoodDBHelper.COL_ID + "=?";
        String[] whereArgs = new String[]{String.valueOf(food.get_id())};

        int result = sqLiteDatabase.update(foodDBHelper.TABLE_NAME, row, whereClaus, whereArgs);
...
//    _id 를 기준으로 DB에서 food 삭제
    public boolean removeFood(long id) {
        ...
        foodDBHelper.close();   //DBHelper 닫아주는것 잊지말기!
